...
const ExperienceItem = ({ organization, profile, startDate, endDate, location, description }: ExperienceProps) => {
    return (<article className="flex justify-between flex-col gap-y-4 lg:flex-row">
        <div className="flex flex-col w-full lg:w-[70%] flex-grow-0">
            ...
            <div className="relative mt-4 px-4 sm:px-0">
                {description?.map((achievement, index) => <h2 key={index} className="list-item text-sm sm:text-base font-light  lg:text-left sm:leading-8">{achievement}</h2>)}
            </div>
        </div>
